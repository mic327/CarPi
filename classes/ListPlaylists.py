...
class ListPlaylists(QWidget):
	def __init__(self,Path,Edit=False):
		...
		#definicja sygnału
		if(self.Path=="/home/pi/Playlists/"):
		...
		for i in range(self.PlaylistCounter):
			...
			if(self.Path=="/home/pi/Playlists/" and self.Edit==False):
				self.ButtonsList[i].setStyleSheet("QPushButton{background:#fe022c url(:/images/edit.png) no-repeat;max-width:50px;min-height:50px;margin-left:500px}")
				self.ButtonsList[i].clicked.connect(lambda: self.editPlaylist())
			#widok dodawania piosenek do playlisty
			if(self.Path=="/home/pi/Music/" and self.Edit==False):
				self.ButtonsList[i].setStyleSheet("QPushButton{background:#168c24 url(:/images/add_small.png) no-repeat;max-width:50px;min-height:50px;margin-left:500px}")
		...
		if(self.Edit==True):
			...
			self.PlaylistFiles=os.listdir("/home/pi/Music/")
			...
			for j in range(self.PlaylistCounter):
				...
				for k in range(len(self.TitlesList)):
					if(self.ModelIndices[j].data()==self.TitlesList[k]):
						...
						self.ButtonsList[l].setStyleSheet("QPushButton{background:#fe022c url(:/images/delete_small.png) no-repeat;max-width:50px;min-height:50px;margin-left:500px}")
				...
				if(Added==False):
					self.ButtonsList.append(QPushButton(self))
					self.ButtonsList[l].setGeometry(QtCore.QRect(0, 0, 50, 50))
					self.ButtonsList[l].setStyleSheet("QPushButton{background:#168c24 url(:/images/add_small.png) no-repeat;max-width:50px;min-height:50px;margin-left:500px}")
	...
	def addToPlaylist(self):
		...
		#stary slot usuwam
		#nowy ustawiam
		self.ButtonsList[ButtonIndex].setStyleSheet("QPushButton{background:#fe022c url(:/images/delete_small.png) no-repeat;max-width:50px;min-height:50px;margin-left:500px}")
	...
	def removeFromPlaylist(self):
		...
		#stary slot usuwam
		#nowy ustawiam
		self.ButtonsList[ButtonIndex].setStyleSheet("QPushButton{background:#168c24 url(:/images/add_small.png) no-repeat;max-width:50px;min-height:50px;margin-left:500px}")
		self.ButtonsList[ButtonIndex].clicked.disconnect()
